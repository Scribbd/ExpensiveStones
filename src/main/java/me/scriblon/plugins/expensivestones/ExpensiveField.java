...
public class ExpensiveField {
	...
	public ExpensiveField(Block sign, Block chest, Field field) {
		if (isCorrectValues(sign, chest)) {
			...
			this.field = field;
			this.settings = ExpensiveStones.getInstance().getESFieldManager()
					.getESFieldSetting(field.getTypeId());
		}
	}
	...
	public ExpensiveField(int status, Location signLocation,
		...
		this.field = PreciousStones.getInstance().getForceFieldManager()
				.getField(fieldLocation.getBlock());
		this.settings = ExpensiveStones.getInstance().getESFieldManager()
				.getESFieldSetting(field.getTypeId());
	}
	...
	public ExpensiveField(Field field) {
		...
		settings = ExpensiveStones.getInstance().getESFieldManager()
				.getESFieldSetting(field.getTypeId());
